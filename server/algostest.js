function algorithm(dataset) {
  ...
  function findPathBetweenFinal(dataset) {
    var graph = dataset.stars;
    var source = dataset.stars[0];
    var target = getStar(dataset.endPoint, graph);
    return findPathBetween(graph, source, target);
  }
  ...
  return findPathBetweenFinal(dataset);
